...
5.  Samples outside the agreement limits are flagged. For potency data these are labelled "Outside LsA" (Outside the Limits of Agreement) and for efficacy data they are labelled "Outlier". Since these values are 2sd outliers, it is expected that 5% of the data to be flagged if the data follow a normal distribution. This is not a flag for automatically excluding data.
...
dataCols <- c('Difference', 'MeanDiff', 'Reference', 'UDL', 'LDL', 'ULA', 'LLA')

plotData <- mdData %>% 
  select(!starts_with('Exp')) %>% 
  mutate(Class = if_else(Difference > mdStats$ULA | Difference < mdStats$LLA, 'flagged', NA),
  ...
  mutate(Lines = if_else(str_ends(DataType,'LA'), 'Agreement Limit',
...
mdConceptPlot <- ggplot(mdData, aes(x = Mean, y = Difference)) +
  ...
  annotate(geom = "rect", xmin = -50, xmax = 150, ymin = mdStats$UDL, ymax = mdStats$ULA,
           fill = "green", linetype = 0) +
  annotate(geom = "rect", xmin = -50, xmax = 150, ymin = mdStats$LLA, ymax = mdStats$LDL,
           fill = "green", linetype = 0) +
  annotate(geom = "rect", xmin = -50, xmax = 150, ymin = mdStats$ULA, ymax = 90,
           fill = "red", linetype = 0) +
  annotate(geom = "rect", xmin = -50, xmax = 150, ymin = -90, ymax = mdStats$LLA,
           fill = "red", linetype = 0) +
  geom_point() +
...
@fig-corrPlot shows the correlation between the two experiments using Spearman's method. This rank correlation is appropriate for efficacy data and log transformed potency data. To view potency data in the original concentration units, both the x-axis and y-axis should be log transformed to see a rank correlation.
...
effCalcData <- read_csv(file.path(reports_dir, 'MSD20Data320_EFF', 'MSD20Data320_EFF_OutputData.csv'), col_types = 'cddddc') %>% 
  slice_head(n = 6) %>% 
  gt() %>% 
...
Once the data have been uploaded, the Replicate-Experiment calculations are displayed as shown in @tbl-effCalcData. This data now includes the Mean and Difference values as well as Class, in addition to the original data. The Class column identifies samples that fall outside the agreement limits with the label "Outlier". This table is fully sortable in the web tool. ***Note. While the input data may contain any number of digits after the decimal point and are used for the analysis, calculated values and statistics are displayed to 3 significant digits*** [@dahlin2019]***.***

![Mean-Difference plot of efficacy data (sd constant).](Reports/MSD20Data320_EFF/MSD20Data320_MSDFig.png){#fig-effMDplot}

The Bland-Altman plot (@fig-effMDplot) shows that the variation is consistent across the range of measured values with most of the difference values close to the reference line at 0. The statistics in the plot are based on the distribution in @fig-diffdist.

```{r}
#| label: tbl-effStats
#| tbl-cap: Efficacy Replicate-Experiment Statistics (sd constant).

effStats <- read_csv(file.path(reports_dir, 'MSD20Data320_EFF', 'MSD20Data320_EFF_Stats.csv'), col_types = 'cdddddd') %>% 
  gt() %>% 
...
![Efficacy Correlation plot (sd constant).](Reports/MSD20Data320_EFF/MSD20Data320_CorrFig.png){#fig-effcorrplot}
...
![Mean Difference plot for an assay with 10% CV.](Reports/MSD_320_CV10_EFF/MSD_320_CV10_MSDFig.png){#fig-effMDcv}
...
effCvStats <- read_csv(file.path(reports_dir, 'MSD_320_CV10_EFF', 'MSD_320_CV10_EFF_Stats.csv'), col_types = 'cdddddd') %>% 
  gt() %>% 
...
![Correlation plot for an assay with 10% CV.](Reports/MSD_320_CV10_EFF/MSD_320_CV10_CorrFig.png){#fig-effConstCVCorr}
...
potCalcData <- read_csv(file.path(reports_dir, 'MSR3data32_POT', 'MSR3data32_POT_OutputData.csv'), col_types = 'cddddc') %>% 
  slice_head(n = 6) %>% 
  gt() %>% 
  tab_header(title = 'Calculated Data') %>% 
  fmt_scientific()
...
The corresponding statistics are shown in @tbl-potStats. There is only a single value labelled as Outside LsA in the Bland-Altman plot. This is consistent with a data set of this size. The ratio for this sample, while outside of the ratio limits, is close and thus not an obvious outlier.

![Potency Correlation plot.](Reports/MSR3data32_POT/MSR3data32_CorrFig.png){#fig-potCorr}
...
![Mean Ratio plot.](Reports/FewSmplRep_POT/FewSmpl_MSRFig.png){#fig-baFewSmplPlot}

```{r}
#| label: tbl-fewSmplStats
#| tbl-cap: Replicate-Experiment Statistics.

fewSmplStats <- read_csv(file.path(reports_dir, 'FewSmplRep_POT', 'FewSmpl_POT_Stats.csv'), col_types = 'cdddddd') %>% 
  gt() %>% 
  tab_header(title = 'Potency Stats') %>% 
  fmt_scientific()
...
![Correlation Plot](Reports/FewSmplRep_POT/FewSmpl_CorrFig.png){#fig-fewCorr}
...
![Replicated samples Mean Ratio plot.](Reports/FewSmplRep_POT/FewSmplRep_MSRFig.png){#fig-baFewSmplsRepsPlot}
...
fewSmplRepStats <- read_csv(file.path(reports_dir, 'FewSmplRep_POT', 'FewSmplRep_POT_Stats.csv'), col_types = 'cdddddd') %>% 
  gt() %>% 
  tab_header(title = 'Potency Stats') %>% 
  fmt_scientific()
...
![Replicated samples correlation plot.](Reports/FewSmplRep_POT/FewSmplRep_CorrFig.png){#fig-repCorr}
...
potShiftData <- read_csv(file.path(reports_dir, 'MSR3data32shift_POT', 'MSR3data32shift_POT_OutputData.csv'), col_types = 'cdddddd') %>% 
  slice_head(n = 6) %>% 
  gt() %>% 
...
![Systematic error Bland-Altman plot.](Reports/MSR3data32shift_POT/MSR3data32shift_MSRFig.png){#fig-potShiftBAPlot}
...
potShiftStats <- read_csv(file.path(reports_dir, 'MSR3data32shift_POT', 'MSR3data32shift_POT_Stats.csv'))

potShiftStatsTbl <- potShiftStats %>% 
  gt() %>% 
  tab_header(title = 'Potency Stats') %>% 
  fmt_scientific()
...
![Systematic Shift Correlation Plot.](Reports/MSR3data32shift_POT/MSR3data32shift_CorrFig.png){#fig-potShiftCorrPlot}
